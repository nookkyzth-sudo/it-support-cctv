...
export function PurchaseTable({ initialData, isAdmin }: { initialData: any[], isAdmin: boolean }) {
  ...
  const uniqueBranches = useMemo(() => {
    ...
    initialData.forEach((item) => {
      if (item.branch?.branch_name) branches.add(item.branch.branch_name);
    });
  ...
  const filteredData = useMemo(() => {
    return initialData.filter((item) => {
      ...
      // Branch filter
      if (branchFilter && item.branch?.branch_name !== branchFilter) return false;

      // Date filter
      if (dateFrom || dateTo) {
        const iDate = new Date(item.request_date);
  ...
  const downloadCsv = () => {
    const headers = ["วันที่แจ้ง", "สาขา", "ผู้แจ้ง", "รายละเอียดทั้งหมด"];
  ...
  const downloadExcel = async () => {
    ...
    const data = filteredData.map(item => ({
      "วันที่แจ้ง": formatDate(item.request_date),
      "สาขา": item.branch?.branch_name || "",
      "ผู้แจ้ง": item.requester?.name || "-",
      "รายละเอียดทั้งหมด": item.items_details || ""
    }));

    const worksheet = XLSX.utils.json_to_sheet(data);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Purchases");
    XLSX.writeFile(workbook, `purchase-report-${new Date().toISOString().slice(0, 10)}.xlsx`);
  };
  ...
  if (initialData.length === 0) {
    return <div className="text-center py-8 text-gray-500">ยังไม่มีข้อมูลการจดบันทึกแจ้งซื้อ</div>;
  }
      ...
      <div className="flex flex-col lg:flex-row items-start lg:items-end justify-between gap-4 mb-4">
        ...
        <div className="flex flex-wrap items-end gap-3 flex-1">
          <div>
            ...
            <input
              type="text"
              placeholder="รายละเอียด, ผู้แจ้ง..."
              value={searchTerm}
          ...
          <table className="w-full text-sm text-left">
            <thead>
              <tr className="border-b border-gray-100 uppercase tracking-wider text-[11px] font-bold text-gray-400">
                <th className="px-4 py-3 pb-4 whitespace-nowrap">วันที่แจ้ง</th>
                <th className="px-4 py-3 pb-4 whitespace-nowrap">สาขา (ผู้แจ้ง)</th>
                <th className="px-4 py-3 pb-4">รายละเอียดทั้งหมด</th>
              ...
              {filteredData.map((item) => (
                 <tr key={item.id} className="border-b border-gray-50 hover:bg-[#F4F7FE]/50 align-top transition-colors">
                  ...
                  <td className="px-4 py-4 font-bold text-[#2B3674] whitespace-nowrap">
                    {item.branch?.branch_name}
                    <div className="text-xs text-gray-400 font-medium mt-0.5">{item.requester?.name}</div>
                  </td>
